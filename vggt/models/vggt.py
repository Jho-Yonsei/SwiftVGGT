...
class VGGT(nn.Module, PyTorchModelHubMixin):
    def __init__(
        ...
        self.aggregator = Aggregator(img_size=img_size, patch_size=patch_size, embed_dim=embed_dim, merging=merging, vis_attn_map=vis_attn_map)
    ...
    def run_heads(self, camera_tokens, patch_tokens, images: torch.Tensor, patch_start_idx: torch.Tensor):
        if len(images.shape) == 4:
            images = images.unsqueeze(0)
    ...
    def run_camera_head(self, camera_tokens):
        predictions = {}
    ...
    def run_depth_head(self, patch_tokens, images: torch.Tensor, patch_start_idx: torch.Tensor):
        if len(images.shape) == 4:
            images = images.unsqueeze(0)

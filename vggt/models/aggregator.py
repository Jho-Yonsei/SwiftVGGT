...
class Aggregator(nn.Module):
    ...
    def forward_DINOv2(self, images: torch.Tensor):
    ...
    def forward_aggregator(self, patch_tokens: torch.Tensor, H, W) -> Tuple[List[torch.Tensor], int]:
        B = 1
        S, P, C = patch_tokens.shape

        # Expand camera and register tokens to match batch size and sequence length
        camera_token = slice_expand_and_flatten(self.camera_token.to(torch.bfloat16), B, S)
        register_token = slice_expand_and_flatten(self.register_token.to(torch.bfloat16), B, S)
        ...
        if self.patch_start_idx > 0:
            ...
            pos_original = pos
            pos = pos + 1
            pos_special = torch.zeros(B * S, self.patch_start_idx, 2, device="cuda", dtype=torch.long)
            ...
            if block_num % 1 == 0:
                ...
                if hasattr(self, "position_getter") and self.position_getter is not None:
                    if hasattr(self.position_getter, "position_cache"):
                        ...
                        if len(current_cache) > 1:  # If there are multiple cache entries
            ...
            for attn_type in self.aa_order:
                if attn_type == "frame":
                    ...
                    tokens, global_idx, global_intermediates = self._process_global_attention(
                        tokens, B, S, P, C, global_idx, pos=pos, global_merging=block_num, need_intermediates=need_intermediates,
                    )
                else:
                    raise ValueError(f"Unknown attention type: {attn_type}")
            
            concat_inter = torch.cat([frame_intermediates[0].detach(), global_intermediates[0].detach()], dim=-1)
            if concat_inter.dtype != torch.bfloat16:
                concat_inter = concat_inter.to(torch.bfloat16)
            all_tokens_list.append(concat_inter)
    ...
    def _process_global_attention(self, tokens, B, S, P, C, global_idx, pos=None, global_merging=None, need_intermediates=False):
        """

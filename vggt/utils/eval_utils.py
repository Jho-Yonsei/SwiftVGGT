...
def process_one(image):
    ...
    # Resize with Pillow-SIMD BICUBIC → same result as original but faster
...
def get_vgg_input_imgs(images, num_workers=32):
    with ThreadPoolExecutor(max_workers=num_workers) as ex:
        out = list(ex.map(process_one, images))

    imgs, heights = zip(*out)
    vgg_imgs = torch.stack(imgs)
    ...
    patch_w = final_w // 14
    patch_h = final_h // 14
    return vgg_imgs, patch_w, patch_h
...
def load_images_rgb(image_paths: List[Path], num_workers=32):
    with ThreadPoolExecutor(max_workers=num_workers) as executor:
        imgs = list(executor.map(load_one, image_paths))
